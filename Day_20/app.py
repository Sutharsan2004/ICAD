...
#         "Click the button above to run AI room detection and show the floor plan.</p>",
#         unsafe_allow_html=True)
#     st.stop()
...
"""
app.py — CAD Room Extractor + Heat Load + CFM Calculator  v20 (Smart Heuristic Edition)
=======================================================================================
v20 Changes:
  - Replaced legacy name-guessing logic with Geometric Layer Scoring.
  - Automatically calculates bounding box spread, average line length, and entity penalties.
  - Ranks layers dynamically and auto-selects the top structural candidates.
"""

import subprocess, os, json, re, io, base64, math
from collections import defaultdict
...
@st.cache_data(show_spinner=False)
def scan_and_score_layers(dxf_path: str):
    """
    Scans layers and calculates a structural geometry score based on:
    1. Line length (structural walls have long continuous lines)
    2. Bounding box spatial spread (walls span the whole drawing)
    3. Entity type penalties (punish blocks, hatches, and text)
    """
    doc = ezdxf.readfile(dxf_path)
    msp = doc.modelspace()
    all_lyrs = {layer.dxf.name for layer in doc.layers}
    ...
    for ent in msp:
        ...
        if lyr not in stats:
            stats[lyr] = {
                "count": 0, "line_len": 0.0, "line_count": 0,
        ...
        # Look for clean geometry to measure
        if dxftype == "LINE":
            try:
                p1, p2 = ent.dxf.start, ent.dxf.end
                lx, ly = min(p1.x, p2.x), min(p1.y, p2.y)
                hx, hy = max(p1.x, p2.x), max(p1.y, p2.y)
                st_data["min_x"] = min(st_data["min_x"], lx)
                st_data["min_y"] = min(st_data["min_y"], ly)
                st_data["max_x"] = max(st_data["max_x"], hx)
                st_data["max_y"] = max(st_data["max_y"], hy)
                length = math.hypot(p2.x - p1.x, p2.y - p1.y)
                st_data["line_len"] += length
                st_data["line_count"] += 1
            except: pass
            ...
            try:
                ...
                if pts:
                    xs = [p[0] for p in pts]
                    ys = [p[1] for p in pts]
                    st_data["min_x"] = min(st_data["min_x"], min(xs))
                    st_data["min_y"] = min(st_data["min_y"], min(ys))
                    st_data["max_x"] = max(st_data["max_x"], max(xs))
                    st_data["max_y"] = max(st_data["max_y"], max(ys))
                    
                    length = 0
                    for i in range(len(pts)-1):
                        length += math.hypot(pts[i+1][0] - pts[i][0], pts[i+1][1] - pts[i][1])
                    st_data["line_len"] += length
                    st_data["line_count"] += len(pts) - 1
            except: pass
        # Penalize non-structural / highly dense detail entities
        elif dxftype in {"INSERT", "SPLINE", "TEXT", "MTEXT", "HATCH", "DIMENSION", "ARC", "CIRCLE"}:
            st_data["bad_ents"] += 1

    # Find the global bounding box size
    # Find the global bounding box size
    for lyr, st_data in stats.items():
        if st_data["min_x"] != float('inf') and st_data["count"] >= 5:
            global_min_x = min(global_min_x, st_data["min_x"])
            global_max_x = max(global_max_x, st_data["max_x"])
            global_min_y = min(global_min_y, st_data["min_y"])
            global_max_y = max(global_max_y, st_data["max_y"])
            
    global_area = max((global_max_x - global_min_x) * (global_max_y - global_min_y), 1.0)
    
    # Calculate normalization factors based on realistic layers
    max_total_len = 1.0
    max_avg_len = 1.0
    
    for lyr, st_data in stats.items():
        st_data["avg_len"] = st_data["line_len"] / max(st_data["line_count"], 1)
        
        # Only let valid structural layers set the benchmarks
        if st_data["count"] >= 5:
            max_total_len = max(max_total_len, st_data["line_len"])
            max_avg_len = max(max_avg_len, st_data["avg_len"])
            
        if st_data["min_x"] != float('inf'):
            st_data["area"] = (st_data["max_x"] - st_data["min_x"]) * (st_data["max_y"] - st_data["min_y"])
        else:
            st_data["area"] = 0.0

    # Calculate final structural scores
    scores = {}
    for lyr, st_data in stats.items():
        # min(..., 1.0) caps values so giant single-line borders don't break the math
        norm_total_len = min(st_data["line_len"] / max_total_len, 1.0)
        norm_avg_len   = min(st_data["avg_len"] / max_avg_len, 1.0)
        norm_area      = min(st_data["area"] / global_area, 1.0)
        penalty        = st_data["bad_ents"] / max(st_data["count"], 1)
        
        # NEW FORMULA: Total amount of drawn "ink" is now the primary driver
        score = (norm_total_len * 0.50) + (norm_avg_len * 0.25) + (norm_area * 0.25) - (penalty * 0.50)
        
        # The Nuclear Penalty: Hard limit for non-structural layers
        if st_data["count"] < 5:
            score -= 2.0 
    ...
    sorted_lyrs = sorted(all_lyrs, key=lambda l: scores.get(l, -999.0), reverse=True)
    count_dict = {l: stats.get(l, {}).get("count", 0) for l in all_lyrs}
    
    return sorted_lyrs, count_dict, all_lyrs, scores
...
def detect_rooms_cached(
    ...
    if use_glass:
        ...
        # --- FIX: SMART AUTO-SCALE FOR METER DRAWINGS ---
        all_xs = [pt[0] for line in raw_wall_lines for pt in line]
        if all_xs:
            span = max(all_xs) - min(all_xs)
            # If the entire building is less than 500 units wide, it must be in Meters or Feet
            if 0 < span < 500:
                print(f"[Auto-Scale] Drawing span is {span:.1f} units. Assuming Meters and scaling to Millimeters.")
                raw_wall_lines  = [[[x*1000.0, y*1000.0] for x,y in line] for line in raw_wall_lines]
                raw_glass_lines = [[[x*1000.0, y*1000.0] for x,y in line] for line in raw_glass_lines]
                raw_furn_lines  = [[[x*1000.0, y*1000.0] for x,y in line] for line in raw_furn_lines]
...
with st.sidebar:
    ...
    st.divider()
    st.markdown("**Layers**")
    wall_input  = st.text_area("Wall / Base layers (Fallback overrides)",
        "Wall\nWalls")  #"MAIN\nMAIN-4\nCEN-1\nA-WALL\nWALLS\nWall\nWalls\nBase\n0"
    glass_input = st.text_area("Glass layers  (blank = Mode B)", "GLASS")
    furn_input  = st.text_area("Furniture / Door layers (optional)",
        "Furniture\nDoors\nFURNITURE\nDOOR\nPlanters")
...
#  ② LAYER SCAN + LAYER SELECTION  (always visible after upload)
# ═════════════════════════════════════════════════════════════════════════════
sorted_layers, layer_entity_count, all_layers_in_file, layer_scores = scan_and_score_layers(dxf_path)
active_layers = [l for l in sorted_layers if layer_entity_count.get(l, 0) > 0]
...
# Exact match for walls to avoid accidental matches (like "0" matching "LAYER-01")
matched_wall  = {x.upper() for x in all_layers_in_file if x.upper() in WALL_LAYERS}

# Substring match for Glass and Furniture
matched_glass = {x.upper() for x in all_layers_in_file if any(kw in x.upper() for kw in GLASS_LAYERS)}
matched_furn  = {x.upper() for x in all_layers_in_file if any(kw in x.upper() for kw in FURN_LAYERS)}

with st.expander("🔍 Smart Layer Selection", expanded=True):
    col1, col2 = st.columns(2)
    with col1:
        st.markdown("**Layers Ranked by Structural Score:**")
        for lyr in active_layers:
            cnt = layer_entity_count.get(lyr, 0)
            score = layer_scores.get(lyr, 0)
            
            # Show visual indicators based on the new smart score
            if score > 0.4: tag = "🟩"
            elif score > 0.1: tag = "🟨"
            else: tag = "⚪"
                
            st.markdown(f"{tag} `{lyr}` — {cnt} entities | Score: {score:.2f}")
    with col2:
        # FIX: The AI will now ignore any layer that you explicitly assigned to Glass or Furniture
        smart_wall_defaults = [
            l for l in active_layers 
            if layer_scores.get(l, 0) > 0.30 
            and l.upper() not in matched_glass 
            and l.upper() not in matched_furn
        ]
        
        # If user hardcoded a direct match in sidebar, respect it. Otherwise, use AI selection.
        wall_default  = [l for l in active_layers if l.upper() in matched_wall] or smart_wall_defaults
        glass_default = [l for l in active_layers if l.upper() in matched_glass]
        furn_default  = [l for l in active_layers if l.upper() in matched_furn]
